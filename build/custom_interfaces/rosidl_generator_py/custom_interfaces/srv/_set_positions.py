...
class SetPositions_Response(metaclass=Metaclass_SetPositions_Response):
    ...
    def __init__(self, **kwargs):
        assert all('_' + key in self.__slots__ for key in kwargs.keys()), \
            'Invalid arguments passed to constructor: %s' % \
            ', '.join(sorted(k for k in kwargs.keys() if '_' + k not in self.__slots__))
        self.torques = array.array('d', kwargs.get('torques', []))
    ...
    def torques(self, value):
        if isinstance(value, array.array):
            assert value.typecode == 'd', \
                "The 'torques' array.array() must have the type code of 'd'"
            self._torques = value
            return
        if __debug__:
            from collections.abc import Sequence
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 all(not (val < -1.7976931348623157e+308 or val > 1.7976931348623157e+308) or math.isinf(val) for val in value)), \
                "The 'torques' field must be a set or sequence and each value of type 'float' and each double in [-.000000, .000000]"
        self._torques = array.array('d', value)
...
class Metaclass_SetPositions(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'custom_interfaces.srv.SetPositions')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
            ...
            from custom_interfaces.srv import _set_positions
...
class SetPositions(metaclass=Metaclass_SetPositions):
    from custom_interfaces.srv._set_positions import SetPositions_Request as Request
    from custom_interfaces.srv._set_positions import SetPositions_Response as Response
